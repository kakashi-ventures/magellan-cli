...
\begin{enumerate}
    ...
    \item \textbf{Identification of measurement censoring as the dominant confound.} The \textit{T.\ thermophilus} artifact was independently predicted by a companion hypothesis (C1-H3: ``Censored GEV Recovers the Invisible 20\%'') from the same session, which proposed applying censored maximum likelihood methods from flood frequency analysis to the TPP measurement window. Implementing censored GEV estimation (analogous to methods used for censored flood records in hydrology) is the most direct path to disentangling biological upper bounds from measurement artifacts, and constitutes the primary follow-up analysis for this verification.
\end{enumerate}
