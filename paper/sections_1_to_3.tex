...
\begin{enumerate}
    ...
    \item Two computational verifications on public data demonstrating that the system generates specific, testable, falsifiable hypotheses: (a)~applying extreme value theory to the Meltome Atlas (48,000 proteins, 13 species)---the first such application, revealing universal Weibull-domain behavior but failing to confirm the predicted correlation with growth temperature due to measurement censoring; and (b)~deriving the Cram\'{e}r-Rao bound for plant gravitropic sensing from published statolith parameters---confirmed, showing plants operate 1--6$\times$ above the fundamental physical limit, the first application of statistical estimation theory to plant gravitropism.
...
\begin{table}[t]
\centering
\caption{Agent architecture. Model assignment follows a clear principle: Opus for deep cross-disciplinary reasoning (Scout, Target Evaluator, Generator, Critic, Quality Gate, Holdout Evaluator, Orchestrator); Sonnet for structured, search-intensive tasks (all others). Effort levels are pinned per agent (Opus: max; Sonnet: high) to guarantee quality regardless of the user's session-level effort setting.}
\label{tab:agents}
\small
...
The Scout is the agent that distinguishes MAGELLAN from systems requiring human-defined research objectives. Operating on Opus at maximum effort, it identifies promising cross-field bridges through 10 strategies:
...
The Critic operates on Opus at maximum effort. Its objective is genuinely adversarial: destroying weak hypotheses. A kill rate of 30--50\% is considered healthy; below 15\% indicates insufficient adversarial pressure; a 0\% kill rate triggers mandatory re-examination. The 9 attack vectors are:
