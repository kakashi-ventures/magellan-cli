...
Six agents have a final-pass section before producing output:

| Agent | Reflection | What it does |
|--------|-----------|---------|
| **Generator** | SELF-CRITIQUE | Writes the counter-argument for parametric claims; calibrates [GROUNDED]/[PARAMETRIC] tagging across citation pairing, direction, compartment, magnitude, protein identity |
| **Critic** | META-CRITIQUE | States kill rate, identifies the strongest reason to kill each SURVIVES, confirms web-search coverage, records citation evidence per survivor |
| **Scout** | TARGET QUALITY CHECK | Enforces non-obviousness, strategy diversity, the exploration slot, meta-insights kill patterns, and the impact floor |
| **Quality Gate** | META-VALIDATION | Enforces the web-search floor, the UNVERIFIABLE-blocks-PASS rule, per-claim verification listing, and the citation audit |
...
Frontier models (Opus 5, GPT-5.5 Pro, Gemini 3.1 Pro) score 91–94%+ on GPQA Diamond, PhD-level questions in biology, physics, and chemistry. This is a 55-point jump from GPT-4 (39%, 2023). Parametric knowledge has improved enormously.
...
- **Reduced MUST/CRITICAL/MANDATORY density**: Opus 5 uses adaptive thinking, so emphatic instructions cause overthinking and token waste. Normal-tone instructions produce better results. Exception: the Orchestrator's functional guardrails (anti-inlining) remain unchanged. Opus 5 follows instructions more literally than earlier models, which amplifies this in two ways: prescriptive prompts are interpreted rigidly, and any internal contradiction between two parts of the same prompt now produces unpredictable behaviour rather than being smoothed over. Auditing each agent for self-contradiction matters as much as tone.
...
- **Additional reflection loops**: RETRIEVAL QUALITY CHECK for the Literature Scout (verifies MCP completeness, paper count, gap analysis specificity) and EVOLUTION QUALITY CHECK for the Evolver (applies the improvement bar: specificity increased, not rephrased).

### Model-specific tuning

The four notes below were harvested for Opus 5 (v5.33) and re-verified on Fable 5.1: same direction, same fixes, with one reversal recorded under "No verification scaffolding". The Fable 5.1 harvest (v5.35 fase 3) is a separate commit from the model migration so that an A/B can attribute a quality change to one or the other; what it found, and what it deliberately did not change, is below.
...
- **No verification scaffolding** (Opus 5): the model verifies its own work unprompted, and explicit verification instructions cause over-verification with no capability gain. Generic self-recheck steps were therefore removed from the six reflection loops. What was kept, deliberately, is any step that gathers EXTERNAL evidence or writes a pipeline artifact: those are the agent's job, not redundant self-checking, and the model does not perform them spontaneously. The rule applied: trim a reflection item when it restates a quality bar already in `<constraints>`; never trim a coverage floor on external tool calls, whose failure branch triggers more real searches. **Reversed on Fable 5.1**: its migration guidance says to KEEP verification instructions when migrating, because the Opus 5 advice to delete them does not carry over (the guidance flags itself as resting on a small number of reports). The rule is therefore retired rather than inverted — nothing new was stripped, and the steps v5.33 removed were not restored blind, since re-adding removed text without an A/B would contaminate the same comparison this split of commits exists to protect.
...
- **Explicit length budgets** (Opus 5): responses and written deliverables run longer than on prior models, and effort is NOT a length lever (lowering effort reduces thinking without reliably shortening output). Every agent that writes a markdown deliverable now carries an explicit word or section budget.
- **Explicit scope discipline** (Opus 5): the model expands task scope on its own judgment. Each agent now states which files it writes and which neighbouring agent's job it must not take over. This matters concretely because `upload-session.mjs` auto-publishes every `.md` in the results directory, so a self-initiated extra file ships to the website.
- **One deliverable draft, not two** (Fable 5.1): at effort `xhigh` and `max` the model tends to compose a long deliverable inside its reasoning and then write it out again as the reply — same result, roughly double the output tokens and the wait. Seven agents run at `max` and every one of them writes a long markdown deliverable, so this is the single most expensive Fable-specific behaviour for this pipeline. The instruction lives at the END of the dispatch prompt, not in the sub-agent files: placement is what makes it hold.
- **Writing density and formatting** (Fable 5.1): the direction reverses from earlier models. Fable 5.1 under-formats rather than over-formats (fewer headers and lists) and writes denser prose with more metaphor in place of literal statement. Since MAGELLAN's markdown deliverables are published to the website, the same dispatch-prompt block asks for the outcome first, literal statement over metaphor, and structure where it aids reading. Anti-formatting language would have to be removed under this guidance; an audit of all 15 agents found none, so nothing was deleted.
- **Prescriptiveness audit** (Fable 5.1): the headline warning for this model is that prompts written for earlier ones are over-prescriptive and lose quality. Measured across the fleet, MAGELLAN largely predates the problem: the GOAL/CONSTRAINTS/STRATEGIES structure already states goals and constraints rather than enumerating steps, and v5.33 had already cut emphatic-marker density to 0–2 per sub-agent. What remains is functional guardrails (use this script, run this in background, check contamination before comparison) — the documented exception. Two defects surfaced instead, both worth fixing precisely because this model reads literally: the Generator's self-critique was numbered 1, 5, 3 and the Target Evaluator's constraints skipped 5. Both renumbered.
- **Not applied, and why** (Fable 5.1): the tool-call batching nudge for agent loops is conditional on measuring the share of assistant turns carrying more than one tool call, and that measurement does not exist yet — the guidance is explicit that adding it blind can produce calls issued before the results they depend on. Asynchronous sub-agent delegation is an architectural change to the dispatch model, not a prompt change. Progress-update and context-anxiety instructions were already in the orchestrator from v5.33 and needed no edit.
...
3. **Opus 4.7/4.8 time horizon 14h30m+ (METR)**: Sustained autonomy at this level is a new capability — no pre-2026 model could operate coherently over these durations. Opus 4.8 improves further on long-horizon agentic work (better long-context handling and compaction recovery), extending this capability further
4. **MCP servers**: Structured retrieval independent of generic web search, reducing fragility
5. **GPT-5.5 Pro as external validator**: Current OpenAI frontier model with 1.05M context, web search and code interpreter tools. Provides empirical literature grounding and computational verification independent of Claude. Reduces the risk of Claude validating its own hallucinations
