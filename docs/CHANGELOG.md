...
## v5.24 — Counter-evidence survives to final.json (14 giugno 2026)

**Motivazione**: la pipeline GENERA materiale avversariale ricco — i 9 attack vector del Critic con verdetto SURVIVE/CONDITIONAL/KILLED, le `claims_failed`/`claims_unverifiable`/`key_risk` del Quality Gate — e lo PERSISTE già come JSON tipizzato in `cycle*-critiqued.json` e `quality-gate.json`. Ma `final.json` (l'artefatto pubblicato autoritativo, costruito leggendo `quality-gate.json` da disco) **strippava ogni campo avversariale**, tenendo solo `verdict`/`composite`/`mechanism`/`supporting_evidence`/`test_protocol`. Questo violava la regola del progetto "Every hypothesis MUST have ... counter-evidence" e perdeva le contraddizioni che hanno plasmato ciascun verdetto: un consumatore di `final.json` vedeva la conclusione sopravvissuta ma non gli attacchi che aveva superato. Re-plumbing a basso rischio di campi già esistenti su disco — nessun nuovo lavoro di pipeline.

**Intervento**: nuovo `scripts/enrich-final-counter-evidence.py` — merge **deterministico** (solo JSON, mai scraping del markdown) che aggancia un blocco `counter_evidence` tipato a ogni ipotesi di `final.json`: verdetto + attacchi per-vettore + survival note del Critic, più `key_risk`/`claims_failed`/`conditions`/`counter_evidence_handling` del Quality Gate, con provenance. L'orchestratore lo invoca come **ultimo writer di final.json** (dopo l'enrichment testuale; vedi `discovery-orchestrator.md` §"Enrich final.json (typed counter-evidence / contradictions)"). Idempotente, con backup `final.json.bak`. Assemblaggio deterministico = immune alla compressione del contesto (nessuna ri-derivazione LLM di fatti già su disco).

**Tolleranza di schema**: gli schemi JSON di Critic e Quality Gate variano tra versioni/cicli del pipeline — nel corpus attuale si osservano **≥3 schemi Critic** (`attacks_summary` cycle-2; `attack_findings`+`severity_per_attack` cycle-1; top-level dict `critiqued_hypotheses`+`attack_results`+`kill_reasons`) e **2 schemi Quality Gate** (`hypotheses` con `claims_failed`/`rubric_scores`; `gated_hypotheses` con `fail_reasons`/`rubric_details`). Lo script normalizza tutte le varianti via candidate-field lookup + matching id esatto / cycle-stripped / titolo, così il blocco si popola indipendentemente dalla versione che ha prodotto la sessione.

**Verifica**: `--check` (non-mutante) su entrambe le sessioni con contenuto in `results/` → **5/5** ipotesi con segnale avversariale recuperato, **0** fallback di markdown-scraping, su tutte le varianti di schema. Esempio concreto: la `C1-H4` CONDITIONAL_PASS (targeted-001) ora porta in `final.json` l'errore regolatorio "500 vs 250 business-day" e le `conditions` pre-registrate dal Quality Gate, prima visibili solo dentro `quality-gate.json`.

**Follow-up (non ancora fatto)**: propagazione del blocco a `ingest.json` / sito web (richiede toccare `upload-session.mjs` + schema sito); allineamento di `README.md` e `methodology-v5.md`.
