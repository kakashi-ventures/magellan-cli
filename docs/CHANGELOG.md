...
5. **Forensics documentato** (`docs/forensics-S026-inline-execution.md`): evidence trail completo, root cause corretta (runtime constraint, non model behavior), scope (S001-S024 OK con vecchie versioni CC; S025/S030/S031/S026 failed con nuove versioni CC), remediation architetturale.
6. **S026 post-hoc recovery**: il 18 aprile ri-dispatchati i 4 post-QG validator (literature-scout, cross-model-validator, convergence-scanner, dataset-evidence-miner) dal top-level dove `Agent` funziona. Aggiornati final-hypotheses.md (Post-QG Amendments section con correzioni reali), session-summary.md (Post-QG Real Validation section), ingest.json. Verdetti QG preservati per consistenza metodologica; 3 hypothesis citation-integrity scores effettivamente scesi da 9 a 7, due hypothesis (C2-H8, E1-C1-H4) sostanzialmente downgraded, due (E1-C1-H2, C2-H7) upgraded.
