...
**Model alias resolution**: Agent frontmatter uses `model: opus` and `model: sonnet` aliases. These resolve to the current-latest models at dispatch time (Opus 4.8 and Sonnet 4.6 as of June 2026). Opus 4.8 inherits the Opus 4.7 request surface unchanged (adaptive-thinking-only, no sampling parameters, no last-turn prefill), so the pipeline runs on 4.8 with no code or prompt changes; Anthropic's guidance is that 4.8 performs well out of the box on existing 4.7 prompts. The pipeline was last empirically validated end-to-end on Opus 4.7 (full 14-agent dispatch preserved, tool usage abundant, PASS + CONDITIONAL_PASS outcomes); an equivalent full-session validation on 4.8 is recommended. The behavioral shifts noted in the Opus 4.8 migration guide (fewer subagents spawned by default, fewer tool calls by default, more literal instruction following) do not materialize in this pipeline because the orchestrator has no WebSearch/WebFetch (cannot inline) and stop-gate hooks deterministically validate output completeness. See `docs/CHANGELOG.md` for per-version migration evidence.
