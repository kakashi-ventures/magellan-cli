...
|---|---|---|---|
| **Scout** | Fable | max | Finds WHERE to look: 10 strategies (incl. structural isomorphism + serendipity), bridge concepts, strategy diversification, exploration slot, rotating creativity constraint, TARGET QUALITY CHECK reflection |
| **Target Evaluator** | Fable | max | Adversarial challenge of Scout targets on 4 axes (popularity bias, vagueness, structural impossibility, local-optima) |
| **Literature Scout** | Fable | high | MCP-first retrieval (Semantic Scholar, PubMed), WebSearch fallback, full-text papers, disjointness verification |
| **Computational Validator** | Fable | high | Programmatic bridge verification: KEGG, STRING, PubMed co-occurrence, back-of-envelope physics |
| **Generator** | Fable | max | Hypotheses from parametric knowledge + literature + computational validation. Bisociation + multi-level abstraction. SELF-CRITIQUE with claim-level verification |
| **Critic** | Fable | max | 9 adversarial attack vectors including claim-level fact verification. META-CRITIQUE reflection. Writes critic_questions |
| **Ranker** | Fable | high | 6-dimension weighted scoring, per-hypothesis table, diversity check, Elo tournament sanity check, cross-domain creativity bonus (+0.5 for 2+ discipline boundaries) |
| **Evolver** | Fable | high | Genetic operations with diversity constraint. EVOLUTION QUALITY CHECK reflection. Conditionally skippable |
| **Quality Gate** | Fable | max | 10-point rubric + web novelty + per-claim grounding verification. META-VALIDATION reflection |
| **Session Analyst** | Fable | high | Post-pipeline meta-learning: strategy performance, kill patterns, bridge type analysis, creativity metrics (disciplinary distance, abstraction level, novelty type) → knowledge/meta-insights.md |
| **Cross-Model Validator** | Fable | high | Calls GPT-5.5 Pro (background submit + poll, reasoning xhigh, web search + code interpreter) + Gemini Deep Research Max (Interactions API agent: google_search + url_context + code_execution, autonomous ~80-160 searches) for independent hypothesis validation. Generates consensus report |
| **Convergence Scanner** | Fable | high | Post-QG: searches ClinicalTrials.gov, NIH Reporter, patents for independent convergence signals. Finds partial mechanism confirmations from sources not consulted by pipeline |
| **Dataset Evidence Miner** | Fable | high | Post-QG: queries bioinformatics databases (HPA, GWAS Catalog, ChEMBL, UniProt, PDB) to verify specific molecular claims in passing hypotheses. Suggests computational follow-up queries |
| **Holdout Evaluator** | Fable | max | Validation framework: compares MAGELLAN output against known post-cutoff discoveries. Contamination check + mechanism similarity scoring |
| **Orchestrator** | Fable | max | Pure dispatcher: guard logic, adaptive cycles, session health, meta-learning metrics, disjointness hard constraint, rotating creativity constraint. No WebSearch/WebFetch |

**Model + effort selection principle**: All fifteen agents run on Claude Fable 5 (`fable`), Anthropic's flagship released June 9 2026. The deep/structured distinction is now carried by effort, not by model tier: effort `max` for deep cross-disciplinary reasoning (Scout, Target Evaluator, Generator, Critic, Quality Gate, Holdout Evaluator, Orchestrator) and effort `high` for structured, search-intensive tasks (Literature Scout, Computational Validator, Ranker, Evolver, Session Analyst, Cross-Model Validator, Convergence Scanner, Dataset Evidence Miner). Effort is pinned per agent (both `max` and `high` are valid Fable 5 levels) to guarantee quality regardless of the user's session-level effort setting. Why Fable 5 across the board: it improves on Opus 4.8 in long-horizon autonomy, first-shot correctness, and parallel subagent delegation. Cost tradeoff: Fable 5 is $10/$50 per MTok, more than Opus 4.8 ($5/$25) and Sonnet 4.6 ($3/$15), so the migration raises Anthropic-side cost ~2x on the deep agents and ~3.3x on the structured agents (~2-3x per session). The structured/search agents are where Fable costs most for the least reasoning benefit.

**Fable 5 fallback principle (life-sciences classifiers)**: Fable 5 runs safety classifiers that can decline biology and life-sciences content (molecular mechanisms, lab methods) and return `stop_reason: "refusal"`. Because MAGELLAN is life-sciences-optimized, the agents that generate or verify molecular content (Generator, Critic, Quality Gate, Literature Scout, Computational Validator, Dataset Evidence Miner, Convergence Scanner) are the most exposed. Two-layer mitigation: (1) the orchestrator Guard Protocol auto-re-dispatches a refusing agent ONCE with a per-invocation model override (`opus` for deep, `sonnet` for structured; the per-call parameter overrides frontmatter, and a refused request is not billed); (2) persistent rollback, i.e. if an agent refuses systematically, set its frontmatter `model:` back to `opus`/`sonnet`. The classifier-free model that would eliminate this entirely is Claude Mythos 5 (`claude-mythos-5`), but it is gated behind Project Glasswing (not GA).

**Model alias resolution**: Agent frontmatter uses the `model: fable` alias, which resolves to Claude Fable 5 at dispatch time. The `opus` and `sonnet` aliases (Opus 4.8 / Sonnet 4.6) remain the documented rollback targets. Fable 5 inherits the same request surface MAGELLAN already relied on (adaptive-thinking-only, no sampling parameters, no last-turn prefill), so the pipeline runs on it with no API or code changes; the migration is alias + prompt-harvest + fallback wiring. The orchestrator runs on the session model selected via `/model` (set it to Fable 5 for `/discover`), since a top-level agent loaded by a command is governed by the session model, not its frontmatter. Validation status: this migration must be empirically validated with a full end-to-end `/discover` session (watch for `stop_reason: refusal` and confirm the auto-fallback fires), plus an A/B holdout comparison against the Opus 4.8 / Sonnet 4.6 baseline. The pipeline was last validated end-to-end on Opus 4.7. See `docs/CHANGELOG.md` for per-version migration evidence.
