...
- **Dispatch model**: `/discover` loads `.claude/agents/discovery-orchestrator.md`
  ...
  Opus 5 guidance also advises against using sub-agents for verification, which
...
|---|---|---|---|
| **Scout** | Opus 5 | max | Finds WHERE to look: 10 strategies (incl. structural isomorphism + serendipity), bridge concepts, strategy diversification, exploration slot, rotating creativity constraint, TARGET QUALITY CHECK reflection |
...
| **Literature Scout** | Opus 5 | high | MCP-first retrieval (Semantic Scholar, PubMed), WebSearch fallback, full-text papers, disjointness verification |
| **Computational Validator** | Opus 5 | high | Programmatic bridge verification: KEGG, STRING, PubMed co-occurrence, back-of-envelope physics |
| **Generator** | Opus 5 | max | Hypotheses from parametric knowledge + literature + computational validation. Bisociation + multi-level abstraction. SELF-CRITIQUE with claim-level verification |
| **Critic** | Opus 5 | max | 9 adversarial attack vectors including claim-level fact verification. META-CRITIQUE reflection. Writes critic_questions |
| **Ranker** | Opus 5 | high | 6-dimension weighted scoring, per-hypothesis table, diversity check, Elo tournament sanity check, cross-domain creativity bonus (+0.5 for 2+ discipline boundaries) |
| **Evolver** | Opus 5 | high | Genetic operations with diversity constraint. EVOLUTION QUALITY CHECK reflection. Conditionally skippable |
| **Quality Gate** | Opus 5 | max | 10-point rubric + web novelty + per-claim grounding verification. META-VALIDATION reflection |
| **Session Analyst** | Opus 5 | high | Post-pipeline meta-learning: strategy performance, kill patterns, bridge type analysis, creativity metrics (disciplinary distance, abstraction level, novelty type) → knowledge/meta-insights.md |
| **Cross-Model Validator** | Opus 5 | high | Calls GPT-5.5 Pro (background submit + poll, reasoning xhigh, web search + code interpreter) + Gemini Deep Research Max (Interactions API agent: google_search + url_context + code_execution, autonomous ~80-160 searches) for independent hypothesis validation. Generates consensus report |
| **Convergence Scanner** | Opus 5 | high | Post-QG: searches ClinicalTrials.gov, NIH Reporter, patents for independent convergence signals. Finds partial mechanism confirmations from sources not consulted by pipeline |
| **Dataset Evidence Miner** | Opus 5 | high | Post-QG: queries bioinformatics databases (HPA, GWAS Catalog, ChEMBL, UniProt, PDB) to verify specific molecular claims in passing hypotheses. Suggests computational follow-up queries |
| **Holdout Evaluator** | Opus 5 | max | Validation framework: compares MAGELLAN output against known post-cutoff discoveries. Contamination check + mechanism similarity scoring |
| **Orchestrator** | Opus 5 | max | Pure dispatcher: guard logic, adaptive cycles, session health, meta-learning metrics, disjointness hard constraint, rotating creativity constraint. No WebSearch/WebFetch |

**Model + effort selection principle**: All fifteen agents run on Claude Opus 5 (`opus`), Anthropic's flagship released July 24 2026. The deep/structured distinction is carried by effort, not by model tier: effort `max` for deep cross-disciplinary reasoning (Scout, Target Evaluator, Generator, Critic, Quality Gate, Holdout Evaluator, Orchestrator) and effort `high` for structured, search-intensive tasks (Literature Scout, Computational Validator, Ranker, Evolver, Session Analyst, Cross-Model Validator, Convergence Scanner, Dataset Evidence Miner). Effort is pinned per agent to guarantee quality regardless of the user's session-level effort setting; all five levels (`low`/`medium`/`high`/`xhigh`/`max`) are valid on Opus 5, whose API default is `high`. Why Opus 5 across the board: versus Opus 4.8 it gains materially in exactly MAGELLAN's domain (organic chemistry +10.2pp, protein prediction +7.7pp, genomics described as behaving "more like a careful scientist than any model"), plus long-horizon agentic work and multi-agent coordination. Cost: $5/$25 per MTok, half of Fable 5's $10/$50, while scoring within 0.5% of Fable 5's peak on CursorBench 3.2 at max effort. Caveat on the saving: Opus 5 runs thinking by default and produces longer output than prior models, so the realised per-session saving is smaller than the halved per-token price. Effort is NOT a length lever on Opus 5; length is controlled by explicit budgets in the agent prompts.

**Refusal fallback principle (safety classifiers)**: A refusal is not an error: it returns HTTP 200 with `stop_reason: "refusal"` and a `stop_details.category` (possibly `null`), so it never appears in error-rate monitoring — branch on `stop_reason`, never on the inner fields. Scope on the current primary: Opus 5 runs **cybersecurity-only classifiers**, so the category to expect is `cyber`, not `bio`. This is a narrower exposure than a Fable-tier primary, whose classifiers cover a broader set including `bio` — the category whose own documentation warns that beneficial life-sciences work can trigger it. Because MAGELLAN is life-sciences-optimized, any move to a Fable-tier primary reintroduces that exposure on Generator, Critic, Quality Gate, Literature Scout, Computational Validator, Dataset Evidence Miner and Convergence Scanner, so the layer is kept regardless of the primary in force. **The mitigation is constrained by a platform asymmetry**: the Agent tool's per-invocation `model` parameter accepts ONLY the four aliases `sonnet`, `opus`, `haiku`, `fable` (a full model ID fails schema validation before dispatch), while agent frontmatter DOES accept full model IDs. Hence: (1) the orchestrator Guard Protocol auto-re-dispatches a refusing agent ONCE with `model: sonnet` — the only alias that is neither circular nor a step up in classifier coverage, and not itself listed as running these classifiers; (2) persistent rollback, if an agent refuses systematically, sets its frontmatter `model:` to a full ID such as `claude-opus-4-8`, which is the only way to reach a model that has no alias. A refusal before any output is not billed; a mid-stream refusal bills the streamed partial. The server-side `fallbacks` API parameter does NOT propagate into model calls made from inside tool execution, which is why this manual orchestrator-level re-dispatch is the correct mechanism for a 15-agent dispatch architecture rather than a workaround.

**Model alias resolution**: Sub-agent frontmatter uses the `model: opus` alias, which resolves to Claude Opus 5 (model ID `claude-opus-5`, no date suffix) at dispatch time. Opus 5 inherits the request surface MAGELLAN already relied on (adaptive-thinking-only, no sampling parameters, no last-turn prefill), so the pipeline runs on it with no API or code changes. Its two breaking changes versus Opus 4.8 are inert here: MAGELLAN never sets `thinking` (now on by default) and never sets `max_tokens`, `temperature`, `top_p`, or `top_k`. **The orchestrator is the exception to the frontmatter rule**: `/discover` loads it into the top-level session, so it runs on the session model, not its own `model:` field. Set `/model opus` before running `/discover`, otherwise an older model coordinates Opus 5 sub-agents; rolling the orchestrator back is likewise a `/model` change, not a file edit. Validation status: this migration is NOT yet validated end-to-end. It needs a full `/discover` session (watching for `stop_reason: refusal` and confirming the `sonnet` re-dispatch fires), plus an A/B holdout comparison. The Fable 5 baseline it replaces was never validated either; the last full end-to-end validation was on Opus 4.7. See `docs/CHANGELOG.md` for per-version migration evidence.
