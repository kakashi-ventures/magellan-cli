...
- **Dispatch model**: `/discover` loads `.claude/agents/discovery-orchestrator.md`
  ...
  dispatching the 14 pipeline sub-agents via `Agent`. Orchestration runs
  top-level by design, not by runtime limitation: recent Claude Code versions do
  allow nested sub-agent dispatch (default depth 3 since v2.1.219), but nesting
  would split the dispatch record across contexts, so a sub-agent's failure would
  become invisible to the Stop hook that validates `state/dispatch-log.json` and
  to the deliverables gate that checks the session's results directory. MAGELLAN
  therefore keeps the fan-out flat: sub-agents carry `disallowedTools: Agent` and
  communicate exclusively via files in `results/{session-id}/`. Anthropic's own
  prompting guidance also advises against using sub-agents for verification, which
  is what a nested Critic or Quality Gate would amount to.
...
|---|---|---|---|
| **Scout** | Fable 5.1 | max | Finds WHERE to look: 10 strategies (incl. structural isomorphism + serendipity), bridge concepts, strategy diversification, exploration slot, rotating creativity constraint, TARGET QUALITY CHECK reflection |
...
| **Literature Scout** | Fable 5.1 | high | MCP-first retrieval (Semantic Scholar, PubMed), WebSearch fallback, full-text papers, disjointness verification |
| **Computational Validator** | Fable 5.1 | high | Programmatic bridge verification: KEGG, STRING, PubMed co-occurrence, back-of-envelope physics |
| **Generator** | Fable 5.1 | max | Hypotheses from parametric knowledge + literature + computational validation. Bisociation + multi-level abstraction. SELF-CRITIQUE with claim-level verification |
| **Critic** | Fable 5.1 | max | 9 adversarial attack vectors including claim-level fact verification. META-CRITIQUE reflection. Writes critic_questions |
| **Ranker** | Fable 5.1 | high | 6-dimension weighted scoring, per-hypothesis table, diversity check, Elo tournament sanity check, cross-domain creativity bonus (+0.5 for 2+ discipline boundaries) |
| **Evolver** | Fable 5.1 | high | Genetic operations with diversity constraint. EVOLUTION QUALITY CHECK reflection. Conditionally skippable |
| **Quality Gate** | Fable 5.1 | max | 10-point rubric + web novelty + per-claim grounding verification. META-VALIDATION reflection |
| **Session Analyst** | Fable 5.1 | high | Post-pipeline meta-learning: strategy performance, kill patterns, bridge type analysis, creativity metrics (disciplinary distance, abstraction level, novelty type) → knowledge/meta-insights.md |
| **Cross-Model Validator** | Fable 5.1 | high | Calls GPT-5.5 Pro (background submit + poll, reasoning xhigh, web search + code interpreter) + Gemini Deep Research Max (Interactions API agent: google_search + url_context + code_execution, autonomous ~80-160 searches) for independent hypothesis validation. Generates consensus report |
| **Convergence Scanner** | Fable 5.1 | high | Post-QG: searches ClinicalTrials.gov, NIH Reporter, patents for independent convergence signals. Finds partial mechanism confirmations from sources not consulted by pipeline |
| **Dataset Evidence Miner** | Fable 5.1 | high | Post-QG: queries bioinformatics databases (HPA, GWAS Catalog, ChEMBL, UniProt, PDB) to verify specific molecular claims in passing hypotheses. Suggests computational follow-up queries |
| **Holdout Evaluator** | Fable 5.1 | max | Validation framework: compares MAGELLAN output against known post-cutoff discoveries. Contamination check + mechanism similarity scoring |
| **Orchestrator** | Fable 5.1 | max | Pure dispatcher: guard logic, adaptive cycles, session health, meta-learning metrics, disjointness hard constraint, rotating creativity constraint. No WebSearch/WebFetch |

**Model + effort selection principle**: All fifteen agents run on Claude Fable 5.1 (full ID `claude-fable-5-1`, released September 1 2026), Anthropic's most capable widely released model. The deep/structured distinction is carried by effort, not by model tier: effort `max` for deep cross-disciplinary reasoning (Scout, Target Evaluator, Generator, Critic, Quality Gate, Holdout Evaluator, Orchestrator) and effort `high` for structured, search-intensive tasks (Literature Scout, Computational Validator, Ranker, Evolver, Session Analyst, Cross-Model Validator, Convergence Scanner, Dataset Evidence Miner). Effort is pinned per agent to guarantee quality regardless of the user's session-level effort setting; all five levels (`low`/`medium`/`high`/`xhigh`/`max`) are valid on Fable 5.1, whose API default is `high`. Why Fable 5.1: it is the tier Anthropic points to for demanding reasoning and long-horizon agentic work, which is what a 15-agent discovery pipeline is. Cost: $10/$50 per MTok, double Opus 5's $5/$25, with cache write at $12.50 per MTok against Opus 5's $6.25 — on a fleet of 15 agents each starting from its own system prompt, the cache write is where the increase is felt. Operational constraint that does NOT carry over from Opus 5: Fable 5.1 is a Covered Model requiring 30-day retention, so an org under zero data retention gets `400 invalid_request_error` on every call unless Anthropic has expressly authorized it. Effort is NOT a length lever; length is controlled by explicit budgets in the agent prompts. Caveat of merit, stated rather than hidden: Anthropic recommends this tier when evals on Opus 5 at higher effort still fall short, and MAGELLAN has never run those evals — see the validation status below.

**Refusal fallback principle (safety classifiers)**: A refusal is not an error: it returns HTTP 200 with `stop_reason: "refusal"` and a `stop_details.category` (possibly `null`), so it never appears in error-rate monitoring — branch on `stop_reason`, never on the inner fields. Detection is on the artifacts, not on the reply: this model returns its between-tool narration as a progress thinking block that is omitted by default, so a refusal is judged by both the phase JSON and the markdown being absent, and a terse reply over two written files is not one. Scope on the current primary: Fable 5.1's classifiers cover a **broader set than Opus 5's cybersecurity-only ones**, including `bio` — the category whose own documentation warns that beneficial life-sciences work can trigger it. Because MAGELLAN is life-sciences-optimized, the migration turns this layer from precautionary into load-bearing, with Generator, Critic, Quality Gate, Literature Scout, Computational Validator, Dataset Evidence Miner and Convergence Scanner the most exposed. **The mitigation is constrained by a platform asymmetry**: the Agent tool's per-invocation `model` parameter accepts ONLY the four aliases `sonnet`, `opus`, `haiku`, `fable` (a full model ID fails schema validation before dispatch), while agent frontmatter DOES accept full model IDs. Hence: (1) the orchestrator Guard Protocol auto-re-dispatches a declining agent ONCE with `model: opus` — correct on a Fable-tier primary because Opus 5 is cyber-only and therefore does not carry the classifier that most plausibly fired, and because unlike `sonnet`/`haiku` it is not a capability downgrade, nor circular as `fable` would be; (2) persistent rollback, if an agent declines systematically, sets its frontmatter `model:` to a full ID — `claude-opus-5` is the previous pipeline baseline, and frontmatter is the only way to reach a model that has no alias. A refusal before any output is not billed; a mid-stream refusal bills the streamed partial. The server-side `fallbacks` API parameter does NOT propagate into model calls made from inside tool execution, which is why this manual orchestrator-level re-dispatch is the correct mechanism for a 15-agent dispatch architecture rather than a workaround.

**Model pinning (full ID, not alias)**: Sub-agent frontmatter pins the full model ID `claude-fable-5-1`, verified to be accepted there. The `fable` alias is deliberately NOT used: Claude Code 2.1.243 kept `fable` resolving to Fable 5 in gateway sessions, so the same alias resolves to different models depending on session type — the silent-mismatch bug class this repo already hit with `opus`. Requires Claude Code >= 2.1.257 (`scripts/version-check-hook.py` warns below that floor). Fable 5.1 inherits the request surface MAGELLAN already relied on (adaptive-thinking-only, no sampling parameters, no last-turn prefill), so the pipeline runs on it with no API or code changes; its breaking changes versus Fable 5 — forced `tool_choice` returns 400, thinking blocks bound to the producing model, history edits invalidating them — are all inert here, since MAGELLAN sets none of those and leaves history management to the Claude Code harness. **The orchestrator is the exception to the frontmatter rule**: `/discover` loads it into the top-level session, so it runs on the session model, not its own `model:` field. Set `/model claude-fable-5-1` before running `/discover`, otherwise a weaker model coordinates Fable 5.1 sub-agents; rolling the orchestrator back is likewise a `/model` change, not a file edit. Validation status: this migration is NOT yet validated end-to-end. It needs a full `/discover` session (watching for `stop_reason: refusal` and confirming the `opus` re-dispatch fires), plus an A/B holdout comparison against Opus 5 — which is also the eval Anthropic's own guidance asks for before choosing this tier. Neither the Opus 5 nor the Fable 5 baseline was ever validated; the last full end-to-end validation was on Opus 4.7. See `docs/CHANGELOG.md` for per-version migration evidence.

**MCP tool access (sub-agent allowlist)**: `tools:` in agent frontmatter is an allowlist, so an agent that specifies `tools:` without MCP patterns silently loses every MCP tool even when the servers are connected at session level and enabled in `.mcp.json`. This silently disabled the documented MCP-first retrieval mandate. Agents that need structured literature retrieval (Literature Scout, Critic, Quality Gate, Convergence Scanner) therefore declare BOTH `mcp__semantic-scholar__*, mcp__pubmed__*` in `tools:` and an `mcpServers:` list. Symptom of a regression: `Error: No such tool available: mcp__pubmed__pubmed_search` in a sub-agent transcript, followed by a silent fallback to WebSearch.
...
- **Cross-model validation** — After Quality Gate, surviving
  ...
  reasoning effort `xhigh`, web search + code interpreter;
