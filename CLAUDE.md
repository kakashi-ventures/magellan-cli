...
**Refusal fallback principle (life-sciences classifiers)**: Opus 5 runs safety classifiers that can decline biology and life-sciences content (molecular mechanisms, lab methods). A refusal is not an error: it returns HTTP 200 with `stop_reason: "refusal"` and a `stop_details.category` such as `bio`, so it never appears in error-rate monitoring. Because MAGELLAN is life-sciences-optimized, the agents that generate or verify molecular content (Generator, Critic, Quality Gate, Literature Scout, Computational Validator, Dataset Evidence Miner, Convergence Scanner) are the most exposed. Two-layer mitigation: (1) the orchestrator Guard Protocol auto-re-dispatches a refusing agent ONCE with a per-invocation override `model: claude-opus-4-8` (full ID, not an alias: `opus` now resolves to Opus 5, the model that just refused). Opus 4.8 is the fallback Anthropic documents for Opus 5 refusals and is not itself listed as carrying these classifiers; a refused request is not billed, so the fallback is cheap. (2) Persistent rollback: if an agent refuses systematically, set its frontmatter `model:` to `claude-opus-4-8`. Note that the server-side `fallbacks` API parameter does NOT propagate into model calls made from inside tool execution, which is why this manual orchestrator-level re-dispatch is the correct mechanism for a 15-agent dispatch architecture rather than a workaround.

**Model alias resolution**: Sub-agent frontmatter uses the `model: opus` alias, which resolves to Claude Opus 5 (model ID `claude-opus-5`, no date suffix) at dispatch time. Opus 5 inherits the request surface MAGELLAN already relied on (adaptive-thinking-only, no sampling parameters, no last-turn prefill), so the pipeline runs on it with no API or code changes. Its two breaking changes versus Opus 4.8 are inert here: MAGELLAN never sets `thinking` (now on by default) and never sets `max_tokens`, `temperature`, `top_p`, or `top_k`. **The orchestrator is the exception to the frontmatter rule**: `/discover` loads it into the top-level session, so it runs on the session model, not its own `model:` field. Set `/model opus` before running `/discover`, otherwise an older model coordinates Opus 5 sub-agents; rolling the orchestrator back is likewise a `/model` change, not a file edit. Validation status: this migration is NOT yet validated end-to-end. It needs a full `/discover` session (watching for `stop_reason: refusal` and confirming the Opus 4.8 fallback fires), plus an A/B holdout comparison. The Fable 5 baseline it replaces was never validated either; the last full end-to-end validation was on Opus 4.7. See `docs/CHANGELOG.md` for per-version migration evidence.
